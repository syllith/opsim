...
test('dealDamage moves one life card to hand', async () => {
  ...
  const res = await dealDamage(s, 'player', 1);
...
test('dealDamage repeats for multiple damage', async () => {
  const s = createInitialState({});
  ...
  const res = await dealDamage(s, 'player', 2);
...
test('dealDamage triggers defeat when life is zero', async () => {
  const s = createInitialState({});
