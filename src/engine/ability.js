/**
 * src/engine/ability.js
 *
 * Pure ability & targeting helpers extracted from the app's action mechanics.
 * Designed to be used both on server and client as part of a shared engine.
 *
 * Exported functions:
 *  - getAbilityTypeLabel(ability)
 *  - listValidTargets(areas, getCardMeta, getTotalPower, sideSpec, targetType, opts)
 *  - resolveActionTargetSide(actionSource, relativeSide)
 *  - evaluateAbilityTargetAvailability(ability, areas, getCardMeta, getTotalPower, actionSource)
 *  - abilityHasAnySelectableTargets(ability, areas, getCardMeta, getTotalPower, actionSource)
 *  - evaluateActivatableAbilities(abilities, params)
 *  - createTargetValidator(...)
 *  - createStateValidator(...)
 *  - completeAbilityActivation(params)
 *
 * NOTE: This file aims to preserve the same behavior and signatures as the existing code,
 * but packaged in a pure fashion so it can be consumed by both server and client.
 */
 ...
 * listValidTargets
 *
 * Returns valid target contexts (side/section/keyName/index/card) given board `areas`,
 * card metadata helpers (`getCardMeta`, `getTotalPower`), a side spec and a target type.
 *
 * opts:
 *  - uniqueAcrossSequence (bool)
 *  - cumulative (array of previously selected targets)
 *  - requireActive (bool)
 *  - requireRested (bool)
 *  - powerLimit (number)
 *  - pendingPowerDeltas (array of { side, section, keyName, index, delta })
 */
...
/**
 * Convert a relative side (self/opponent/both) into absolute side(s),
 * given the controller in actionSource (which includes side).
 */
...
/**
 * Evaluate whether an ability has targets available. Returns:
 *  { hasTargetRequiringActions, anyTargets, allRequiredAvailable }
 */
export function evaluateAbilityTargetAvailability(
  ...
  // Extract targeting parameters from a structured action (currently supports modifyStat)
...
/* --------------------------
 * Quick selector: any selectable targets
 * -------------------------*/

/**
 * Returns true if ability has any selectable targets (fast check used by UI)
 */
export function abilityHasAnySelectableTargets(
  ...
  try {
    ...
    // No structured actions -> no selectable targets
 ...
 *   - getCardMeta
 *   - getTotalPower
 *   - resolvingAbilityIndex
 *
 * returns array of abilities with added fields:
 *   { index, canActivate, reason, type, typeKey, condition, frequency, ...original }
 */
export function evaluateActivatableAbilities(abilities, params) {
  ...
  return abilities.map((ability, index) => {
    // Supported actions: enumerate acceptable types
    ...
    // Determine if the card instance is currently rested
    ...
    try {
      ...
      // Fail-safe: don't restrict on evaluation errors
    }
    ...
    // DON requirement from condition
...
/* --------------------------
 * Target validators used by UI targeting helpers
 * -------------------------*/

/**
 * createTargetValidator(targetType, action, cumulativeTargets, getTotalPower, getCardMeta, pendingPowerDeltas)
 *
 * Returns a function (card, ctx) => bool for validating a candidate target.
 */
...
/**
 * createStateValidator(targetType, requireRested, ...)
 * Extends target validator by checking rested/active state.
 */
...
/* --------------------------
 * Ability resolution side effects (callbacks)
 * -------------------------*/

/**
 * completeAbilityActivation(params)
 *
 * Performs post-resolution bookkeeping (mark ability used, pay costs, move/trash cards, etc.)
 *
 * params:
 *  - ability
 *  - abilityIndex
 ...
 *  - cardIndex
 *  - setAbilityUsed (fn)
 *  - returnCardToDeck (fn side, section, keyName, index, position)
 *  - removeCardByEffect (fn)
 *  - restCard (fn)
 *  - payLife (fn)
 *  - markAbilityUsed (fn actionSource, index)
 *  - lockCurrentAttack (fn)
 *  - effectApplied (bool)
 *
 * This function intentionally expects caller-provided side-effect helpers so it stays testable.
 */
export function completeAbilityActivation(params) {
  ...
  // Mark used for Once Per Turn abilities and On Play
  if (isOncePerTurn || isOnPlay) {
    if (typeof setAbilityUsed === 'function') {
  ...
  // Lock current attack if needed (when ability applied an effect while attacking)
