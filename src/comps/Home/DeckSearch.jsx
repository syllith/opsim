...
export function useDeckSearch({
  ...
  const start = useCallback((searchConfig) => {
    ...
    setConfig({
      ...
      quantity: lookCount,
      filter: filter || {},
      minSelect: _.isNumber(minSelect) ? minSelect : 0,
      maxSelect: _.isNumber(maxSelect) ? maxSelect : 1,
  ...
  const cardSelectability = useMemo(() => {
    return cards.map((card) => {
      ...
      if (filter.type) {
        const types = _.get(meta, 'types', []);
        const needle = _.toLower(filter.type);
        const typeMatch = _.some(types, (t) => {
          const lower = _.toLower(t);
      ...
      //. Check color filter
      if (filter.color) {
        const colors = _.get(meta, 'colors', []);
        const colorMatch = _.some(
          colors,
          (c) => _.toLower(c) === _.toLower(filter.color)
        );
        if (!colorMatch) {
      ...
      if (filter.attribute) {
        const attr = _.toLower(_.get(meta, 'attribute', ''));
        if (attr !== _.toLower(String(filter.attribute))) {
          return {
            selectable: false,
            reason: `Must have ${filter.attribute} attribute`
          };
        }
      }
      ...
      if (_.isNumber(filter.maxCost)) {
        ...
        if (cardCost > filter.maxCost) {
          return {
            selectable: false,
            reason: `Cost must be ${filter.maxCost} or less`
          };
        }
      }
  ...
  const handleToggleSelect = useCallback((index) => {
    ...
    if (
      newSelected.length >= minSelect &&
      newSelected.length <= maxSelect &&
      !isCurrentlySelected
    ) {
      setTimeout(() => {
        const selectedCards = newSelected.map((i) => cards[i]);
        const remainderIndices = cards
          .map((_, i) => i)
          .filter((i) => !newSelected.includes(i));
        const remainder = remainderIndices.map((i) => cards[i]);

        handleConfirm(selectedCards, remainder);
      }, 150);
    }
  }, [cardSelectability, selected, maxSelect, minSelect, cards, handleConfirm]);

  const selectableCount = useMemo(
    () => cardSelectability.filter((c) => c.selectable).length,
    [cardSelectability]
  );
  ...
  const calculatedWidth = Math.max(
    minWidth,
    Math.min(cardDisplayWidth + 32, 1200) // 32 for padding
  );
          ...
          >
            ...
            <Box sx={{ mb: 1, px: 1, maxWidth: '100%' }}>
              ...
              >
                ...
                {minSelect === 0 &&
                  maxSelect > 0 &&
                  ` Click to select up to ${maxSelect}.`}
                {minSelect > 0 &&
                  ` Click to select ${minSelect === maxSelect
                    ? minSelect
                    : `${minSelect}-${maxSelect}`
                  }.`}
              ...
              >
                {returnLocation === 'bottom' &&
                  `Rest goes to bottom${canReorder ? ' (any order)' : ''
                  }`}
