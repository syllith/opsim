...
// DON!! card constants
export const DON_FRONT_CONSTANT = { id: 'DON', full: '/api/cards/assets/Don/Don.png', thumb: '/api/cards/assets/Don/Don.png' };
export const DON_BACK_CONSTANT = { id: 'DON_BACK', full: '/api/cards/assets/Card%20Backs/CardBackDon.png', thumb: '/api/cards/assets/Card%20Backs/CardBackDon.png' };
...
export const useDonManagement = ({ 
    ...
    const donPhaseGain = useCallback((side, count) => {
        ...
        setAreas((prev) => {
            ...
            const moved = Array.from({ length: toMove }, () => ({ ...DON_FRONT_CONSTANT, rested: false }));
        ...
        setAreas((prev) => {
            ...
            if (section === 'char' && keyName === 'char') {
                ...
                if (donUnderArr.length) {
                    ...
                    appendLog(`[K.O.] Returned ${donUnderArr.length} DON!! to cost area.`);
                    if (Array.isArray(sideLoc.charDon)) {
    ...
    const initializeDonDecks = useCallback(() => {
        setAreas((prev) => {
            const next = structuredClone(prev);
            // DON!! decks (10 each)
            next.player.bottom.don = Array.from({ length: 10 }, () => ({ ...DON_BACK_CONSTANT }));
            next.opponent.top.don = Array.from({ length: 10 }, () => ({ ...DON_BACK_CONSTANT }));
            // Cost areas empty
